...
function Process(action, start, tick, finish)
    ...
    Action = action
	if Action.icon then
		if sharedItems[tostring(Action.icon)] then
			local img = "nui://qbr-inventory/html/" -- default qb-core inventory link
			if not string.find(sharedItems[tostring(Action.icon)].image, "http") then -- 👀 Slipped in support for custom html links too
				if not string.find(sharedItems[tostring(Action.icon)].image, "images/") then --search for if the icon images have /images in the listing
					img = img.."images/"
				end
				Action.icon = img..sharedItems[tostring(Action.icon)].image
			end
		end
	end
